...
/** Parse the ratio filename to suggest a report name.
 *  Pattern: "Report - {Company} - {Date} ..." or just use the stem. */
function suggestReportName(filename: string): string {
  const stem = filename.replace(/\.[^.]+$/, '') // remove extension
  // Try to match "Report - Company Name - YYYY-MM-DD ..."
  const m = stem.match(/^Report\s*-\s*(.+?)\s*-\s*(\d{4}-\d{2}-\d{2})/)
  const today = new Date().toISOString().slice(0, 10)
  if (m) {
    const company = m[1].trim()
    const fyDate = m[2]
    return `${company} - FCR - FY${fyDate} - Generated ${today}`
  }
  // Fallback: use the stem + generation date
  return `${stem} - FCR - Generated ${today}`
}
...
function UploadView() {
  ...
  const handleRatioChange = (files: File[]) => {
    setRatioFiles(files)
    if (files.length > 0 && !reportName) {
      setReportName(suggestReportName(files[0].name))
    }
  }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Company Name (Reg No) - FCR - FY2025-02-28 - Generated 2026-02-09"
            className="w-full border border-gray-300 rounded-lg p-2.5 text-sm"
          />
          <p className="text-xs text-gray-400 mt-1">
            Auto-suggested from the uploaded filename. Edit to include registration number or other details.
          </p>
        </div>
